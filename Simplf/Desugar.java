...
public class Desugar implements Expr.Visitor<Expr>, Stmt.Visitor<Stmt> {
    ...
    public Stmt visitForStmt(For stmt) {
        ...
        // Convert init expression to a statement if it exists
        Stmt initial = (stmt.init != null) ? new Stmt.Expression(stmt.init) : null;

        // Create the loop body, appending incr if it exists
        List<Stmt> bodyStatements = new ArrayList<>();
        bodyStatements.add(stmt.body);
        if (stmt.incr != null) {
            bodyStatements.add(new Stmt.Expression(stmt.incr));
        }
        Stmt.Block loopBody = new Stmt.Block(bodyStatements);

        // Create the while loop with the given condition and the new loop body
        Stmt.While whileLoop = new Stmt.While(stmt.cond, loopBody);

        // Wrap initialization and the while loop in a block
        if (initial != null) {
            return new Stmt.Block(List.of(initial, whileLoop));
        } else {
            return whileLoop; // No init means we can return just the while loop
        }
    }
